...
export class HseRoutinesService {
  ...
  async findAllCategories(options: IFindOptions) {
    ...
    const response = await this.hseRoutineCategoriesRepository.findAndCountWithLang(findOptions);

    const companyFindOptions: FindManyOptions<CompanyHseRoutineCategory> & FindWithLangOptions = { lang: options.lang };
    ...
    const companyResponse = await this.companyHseRoutineCategoriesRepository.findAndCountWithLang(companyFindOptions);

    const combinedData = [...response.data, ...companyResponse.data.map(el => ({ ...el, isCompanyCategory: true }))];
  ...
  async findAllTemplates(options: IFindOptions) {
    const findOptions: FindWithLangOptions = { lang: options.lang };
    if (options.search) findOptions.search = { keys: ['name'], value: options.search };
    const response = await this.hseRoutineTemplatesRepository.findAndCountWithLang(findOptions);

    const companyFindOptions: FindManyOptions<CompanyHseRoutineTemplate> & FindWithLangOptions = { lang: options.lang };
    if (options.user?.companyUuid) companyFindOptions.where = { companyUuid: options.user.companyUuid };
    if (options.search) companyFindOptions.search = { keys: ['name'], value: options.search };
    const companyResponse = await this.companyHseRoutineTemplatesRepository.findAndCountWithLang(companyFindOptions);
